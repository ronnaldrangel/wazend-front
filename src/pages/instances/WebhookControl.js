...
const WebhookControl = ({ instanceName, serverUrl }) => {
    ...
    return (
        <section className='rounded-lg bg-card border border-border shadow-[0_0_5px_rgba(0,0,0,0.1)]'>
            ...
            <div className='p-4'>
                <h3 className='text-sm font-semibold text-foreground mb-4'>Webhook Events</h3>
                    ...
                    {events.map((event) => (
                        <div key={event} className='flex items-center'>
                            <input
                                type='checkbox'
                                id={event}
                                className='mr-2 h-4 w-4 rounded border-border text-emerald-600 focus:ring-emerald-600'
                                checked={selectedEvents.includes(event)}
                                onChange={() => handleEventChange(event)}
                            />
                            <label htmlFor={event} className='text-sm font-semibold text-muted-foreground'>
                                {event}
                            </label>
                        </div>
                    ))}
            ...
            <div className='px-4 pb-4'>
                <Button
                    onClick={handleSave}
                    className={`${webhookUrl ? '' : 'cursor-not-allowed'
                        }`}
            ...
            {/* <div className='p-4'>
                <h3 className='text-sm font-semibold text-foreground mb-2'>Datos del webhook</h3>
                <pre className='text-xs text-muted-foreground bg-muted p-2 rounded-md'>{JSON.stringify(webhookData, null, 2)}</pre>
            </div> */}
        </section>
    );
};
