  ...
  useEffect(() => {
    if (status === "authenticated" && session?.jwt) {
      const checkJwtValidity = async () => {
        try {
          // Agregamos un timeout a la solicitud para manejar cuando el backend no responde
          ...
          // Si hay un error de timeout o el backend no responde, cerramos sesión
          if (error.name === 'AbortError') {
            console.log("Backend no responde (timeout), cerrando sesión...");
            signOut({ callbackUrl: "/login" });
          } else {
            // Para otros tipos de errores (como problemas de red), también cerramos sesión
            console.log("Error de conectividad con el backend, cerrando sesión...");
            signOut({ callbackUrl: "/login" });
          }
        }
      };
